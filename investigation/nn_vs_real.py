...
for far in far_s:
    ...
    data = [pin, Tin, p_ratio, d.cycle, d.thermo, d.cooling, d.opposed, cr, bore, d.bsr,
    ...
    output_real = T4, air_flow, p_max, T_max, indicated_power*1e-3, heat_loss*1e-3, p_tdc*1e-5
...
ax8.plot(far_s, outputs_nn[:, 0], label="nn")
ax8.plot(far_s, outputs_real[:, 0], label="simulation")
ax8.set_title('Tout, bore')
plt.legend()

fig, ax9 = plt.subplots()
ax9.plot(far_s, outputs_nn[:, 1], label="nn")
ax9.plot(far_s, outputs_real[:, 1], label="simulation")
ax9.set_title('air flow, bore')
plt.legend()

fig, ax10 = plt.subplots()
ax10.plot(far_s, outputs_nn[:, 2], label="nn")
ax10.plot(far_s, outputs_real[:, 2], label="simulation")
ax10.set_title('p_max, bore')
plt.legend()

fig, ax11 = plt.subplots()
ax11.plot(far_s, outputs_nn[:, 3], label="nn")
ax11.plot(far_s, outputs_real[:, 3], label="simulation")
ax11.set_title('Tmax, bore')
plt.legend()

fig, ax12 = plt.subplots()
ax12.plot(far_s, outputs_nn[:, 4], label="nn")
ax12.plot(far_s, outputs_real[:, 4], label="simulation")
ax12.set_title('power, bore')
plt.legend()

fig, ax13 = plt.subplots()
ax13.plot(far_s, outputs_nn[:, 5], label="nn")
ax13.plot(far_s, outputs_real[:, 5], label="simulation")
ax13.set_title('heat loss, bore')
plt.legend()

fig, ax14 = plt.subplots()
ax14.plot(far_s, outputs_nn[:, 6], label="nn")
ax14.plot(far_s, outputs_real[:, 6], label="simulation")
