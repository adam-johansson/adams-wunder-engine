...
def run_piston_engine(indata, flags):
    ...
    h_in, _, _, _, R_in, _, _, _ = thermo.mixture(
        t=T_in, p=p_in, equ=0, fuel_type=fuel_type
    )  # assuming pure air in intake
    ...
    for i in range(it):
        ...
        if cycle == "2T":
            ...
            try:
                sol = solve_ivp(
                    ...
                    rtol=1e-10,
                    atol=1e-12,
                )  # 1e-12 needed to not fuck up with latest limits
        ...
        if "validation" not in flags:
            if cycle == "2T":
                ...
                m_air_inlet_closing = m[-1][-1] / (1 + far_s * equ[-1][-1])
        ...
        def find_tout(t):
            h, _, _, _, _, _, _, _ = thermo.mixture(
                t[0], p_out, far_goal / far_s, fuel_type
            )
        ...
        if i > 4:
            ...
            for j in range(1, 3):
                if (
                    ...
                    or mdiff[-j] > m_lim
                    or Tdiff[-j] > T_lim
                    or T_out_diff[-j] > T_out_lim
                    or mf_diff[-j] > mf_lim
                    or equdiff[-j] > equ_lim
                    or mEP_diff[-j] > mEP_lim
                    or mIP_diff[-j] > mIP_lim
                ):
                    convergence = False
        ...
        if i + 1 == it:
            if "sweep" in flags:
                print(
                    f"Simulation never converged. p_in, T_in, cr, far_goal, rpm, bore: {p_in * 1e-5, T_in, cr, far_goal, rpm, d}"
                )
    ...
    if "sweep" not in flags:
        fmep, fmep_aux, fmep_pe_loss = post_processing.friction_patton(
            d, rpm, s, v_mean, p_in, cr, cylinders, lv_max
        )
        bmep = imep - fmep
    ...
    if "sweep" not in flags:
        friction_power_engine = (
            fmep * Vd_tot * rps / n_r
        )  # power lost from piston to crankshaft
    ...
    if "validation" not in flags:
        ...
        T_z1, m_z1, p_z1, V_z1, lambda_z1, phi_z1, equ_hp, T_z2 = twozone_model.twozone(
            phi,
            P[-1],
            T[-1],
            V[-1],
        ...
        nox = nox_model.nox_calculations(
            T_z1,
            m_z1,
            p_z1,
            V_z1,
    ...
    if "sweep" not in flags:
        ...
        if "output_all" and "validation" in flags:
            ...
            output_power_validation(
                power, imep, friction_power, fmep, break_power, bmep, heat_loss_single
            )
