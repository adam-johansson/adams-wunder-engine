...
def sweep_no_diesel_kth_validation(d, flags):
    ...
    for far_goal, p_in in zip(fuel_air_ratios, p_ins):
        ...
        m_wiebe_adjusted = m_wiebe * ((p_dp * T_dp) / (p_in * T_in))

        data = [p_in, T_in, p_ratio, d.cycle, d.thermo, d.cooling, d.opposed, d.cr, d.d, d.bsr,
    ...
    nox_val_exp_file = os.path.join(dirname, '../../validation_output_data/Scania/nox_exp.txt')
    nox_val_sim_file = os.path.join(dirname, '../../validation_output_data/Scania/nox_sim.txt')
    ...
    #ax5.plot(p_ins, nox_val_sim, label="Simulation validation")
