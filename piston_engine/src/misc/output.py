...
def output_power(
    power, imep, friction_power, fmep, break_power, bmep, heat_power, equ_avg
):
...
def output_scavenging_validation(
    purity,
    residual_fraction,
    eta_trapping,
    eta_charging,
    ...
    print(
        f"Scavenging efficency: {eta_sc}. Validation: 0.6965, Error: {eta_sc / 0.6965}"
    )
    print(f"Equivalence ratio based on fuel and air flow {equ_avg}")
    return


def output_power_validation(
    power, imep, friction_power, fmep, break_power, bmep, heat_power
):
    print(
        f"Gas power on the piston: {power * 1e-3} [kW], NASA: 161.15 kW, Error: {power * 1e-3 / 161.15}"
    )
...
def output_efficiencies_validation(
    eta_th,
    hl,
):
    print(
        f"Thermal efficiency: {eta_th * 100} [%], NASA 37.38%, Error: {eta_th * 100 / 37.38}"
    )
    print(f"Percentage heat loss: {100 * hl} [%], NASA 8.11%, Error: {hl * 100 / 8.11}")
    print(
        f"Percentage energy to exhaust: {100 - 100 * hl - 100 * eta_th} [%], NASA 54.52%"
    )
    return
...
def output_thermo_validation(phi, P, T, T_out, air_flow, m_in_IP, fuel_flow, mf):
    print(
        f"Mass flow of air: {air_flow * 1000} [g/s], NASA 296.83 g, Error: {air_flow * 1000 / 296.83}"
    )
    print(
        f"Mass of air inducted per cycle: {m_in_IP[-1][-1] * 1e3} [g], NASA 2.90917 g"
    )
    ...
    print(
        f"Outflow temperature: {T_out} [K], NASA  1143.05 K, Error: {T_out / 1143.05}"
    )
    return
