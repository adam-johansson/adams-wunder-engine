...
def dqdphi(T,P,V,gamma,Twalls,Awalls,v_mean,d,V_d,ref,dtdphi,type1,type2):
    ...
    for (i, j) in zip(Twalls, Awalls):
        ...
        C = k1 * (2.280 * v_mean + 0.308 * v_u) + c2 * V_d * (Tref / (Pref * Vref)) * (Pbar - Pmotor_bar)
...
def dqdphi_h2(t, p, twalls, awalls, bore, v_mean, ref, dqfdphi, dtdphi):
    ...
    alpha = c1 * bore ** (-0.2) * t ** (-0.53) * p ** 0.8 * (v_mean + c2 * (Tref / (Pref * Vref)) * dqfdt) ** 0.8
...
def dqdphi_h2_shudo(t, p, V, twalls, awalls, bore, v_mean, ref, dpdphi, dVdphi, gamma, dtdphi):
    ...
    if v_mean + c2 * (Tref / (Pref * Vref)) * dqdt_apparent < 0:
        return 0.0
    alpha = c1 * bore ** (-0.2) * t ** (-0.53) * pbar ** 0.8 * (v_mean + c2 * (Tref / (Pref * Vref)) * dqdt_apparent) ** 0.8
