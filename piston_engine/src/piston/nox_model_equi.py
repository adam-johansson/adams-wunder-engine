...
def nox_hcci(
    ...
    i = 0
    for T,p,equ in zip(temperatures, pressures,equs):

        xi_N2_0, xi_O2_0, xi_CO2_0, xi_H2O_0, _, _ = molar_fractions(
        equivalence_ratio=equ, fuel_type=fuel_type
    )
        gas.TPX = (
            T,
            p,
    ...
    _, _, _, _, _, _, _, M_global = mixture(t_dummy, p_dummy, equ_global, fuel_type)
    _, _, _, _, _, _, _, M_hot_zone = mixture(t_dummy, p_dummy, equs[-1], fuel_type)
