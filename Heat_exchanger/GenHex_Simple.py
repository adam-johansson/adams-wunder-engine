...
def GetProperties_single(f, dataSource="Coolprop", verbose=False):
    ...
    if (dataSource == "LOCAL") and (f.fluid == "REFPROP::Parahydrogen"):
        ...
        f.k_mean[-1] = CP.PropsSI(
            "CONDUCTIVITY", "P", f.p0_mean, "T", f.T0_mean, f.fluid
        )
        ...
        f.k_mean[-1] = CP.PropsSI(
            "CONDUCTIVITY", "D", f.rho_mean, "T", f.T0_mean, f.fluid
        )
...
def GenHex_single(f1, f2, HEX, verbose=True):
    ...
    for i in range(numIter):
        ...
        HEX.U_f1 = 1 / (
            1 / (HEX.f1_eta_O * HEX.f1_h_ave)
            + HEX.wt / ((HEX.f1_alpha + HEX.f2_alpha) / (2 * HEX.f1_alpha) * HEX.k)
            + 1 / (HEX.f2_eta_O * HEX.f2_h_ave * HEX.f2_alpha / HEX.f1_alpha)
        )
        ...
        if HEX.flow == "Counter":
            # For counter flow
            HEX.eps = (1 - np.exp(-HEX.NTUmax * (1 - HEX.Cr))) / (
                1 - HEX.Cr * np.exp(-HEX.NTUmax * (1 - HEX.Cr))
            )
            ...
            HEX.eps = HEX.NTUmax / (
                HEX.NTUmax / (1 - np.exp(-HEX.NTUmax))
                + HEX.Cr * HEX.NTUmax / (1 - np.exp(-HEX.NTUmax * HEX.Cr))
                - 1
            )
        else:
        ...
        f1_dp0 = (
            ...
            * 1
            / (HEX.f1_Afr) ** 2
            * (
                (1 / HEX.f1_sigma**2 + 1) * (f1.rho[-2] / f1.rho[-1] - 1)
                + HEX.f1_f_ave
                * HEX.Lx
                * HEX.Ly
                * HEX.Lz
        ...
        f2_dp0 = (
            ...
            * 1
            / (HEX.f2_Afr) ** 2
            * (
                (1 / HEX.f2_sigma**2 + 1) * (f2.rho[-2] / f2.rho[-1] - 1)
                + HEX.f2_f_ave
                * HEX.Lx
                * HEX.Ly
                * HEX.Lz
