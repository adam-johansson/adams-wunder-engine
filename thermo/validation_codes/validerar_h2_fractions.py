...
equs = np.linspace(0.1, 0.99, num)
temps = np.linspace(298, 3000, num)

fracs = np.zeros((num, 7))
...
for equ_sc in equs:
    ...
    xi_N2_0, xi_CO2, xi_H2O_0, xi_CO, xi_O2_0, xi_OH_0, xi_H2_0, xi_O_0, xi_H_0 = (
        cantera_combustion_fractions.molar_fractions_combustion(
            T_soc, p_soc, equ_sc, equ_combustion, fuel_type
        )
    )
    ...
    gas.TPX = (
        T,
        p_soc,
        f"H2O:{xi_H2O_0}, O2:{xi_O2_0}, N2:{xi_N2_0}, OH:{xi_OH_0}, H2:{xi_H2_0}, O:{xi_O_0}, H:{xi_H_0} ",
    )
    ...
    fracs[i, :] = np.array([xi_N2, xi_H2O, xi_O2, xi_OH, xi_H2, xi_O, xi_H]) * 100
    ...
    # print(xi_N2_dir + xi_H2O_dir + xi_O2_dir + xi_OH_dir + xi_H2_dir + xi_O_dir +xi_H_dir)
...
ax1.set_xlabel(r"Temperature", fontsize=fs)
...
ax2.set_xlabel(r"Temperature", fontsize=fs)
