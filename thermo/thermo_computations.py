    ...
    if equ == 0 and fuel_equ_ratio == 0:
        ...
        if pure_fuel:
            if fuel_type == 'jetA':
                ...
                N = 4.75 * equ + 17.75 * (1 + x_N2_air / x_O2_air + x_Ar_air / x_O2_air) + fuel_equ_ratio  # total number of moles in gas
                ...
                N = - 0.5 * equ + 0.5 * (1 + x_N2_air / x_O2_air + x_Ar_air / x_O2_air) + fuel_equ_ratio  # total number of moles in gas
...
def equivalence_derivative(equ, t, p, fuel_type, pure_fuel, fuel_equ_ratio):
    ...
    if pure_fuel:
        if fuel_type == 'jetA':
            ...
            Ntot = (4.75 * equ + 17.75 * (1 + x_N2_air / x_O2_air + x_Ar_air / x_O2_air)
                    + fuel_equ_ratio)  # total number of moles in gas
            dNtotdequ = 4.75
            ...
            M = x_N2 * M_N2 + x_O2 * M_O2 + x_CO2 * M_CO2 + x_H2O * M_H2O + x_Ar * M_Ar + x_JETA * M_JETA
            ...
            dellmu_JETAdellequ = (dellx_JETAdellequ * M - x_JETA * dellMdellequ) * (M_JETA / M ** 2)

            dellhdellequ = h_N2 * dellmu_N2dellequ + h_O2 * dellmu_O2dellequ + \
                           h_CO2 * dellmu_CO2dellequ + h_H2O * dellmu_H2Odellequ + \
                           h_Ar * dellmu_Ardellequ + h_JETA * dellmu_JETAdellequ

            delludellequ = dellhdellequ - dellRdellequ * t

        elif fuel_type == 'H2':
            Ntot = - 0.5 * equ + 0.5 * (
                        1 + x_N2_air / x_O2_air + x_Ar_air / x_O2_air) + fuel_equ_ratio  # total number of moles in gas
            dNtotdequ = - 0.5
            ...
            dellx_H2Odellequ = (df4dequ * Ntot - f4 * dNtotdequ) / (Ntot ** 2)
            dellx_Ardellequ = (df5dequ * Ntot - f5 * dNtotdequ) / (Ntot ** 2)
            dellx_H2dellequ = (df6dequ * Ntot - f6 * dNtotdequ) / (Ntot ** 2)
            ...
            dellmu_H2Odellequ = (dellx_H2Odellequ * M - x_H2O * dellMdellequ) * (M_H2O / M ** 2)
            dellmu_Ardellequ = (dellx_Ardellequ * M - x_Ar * dellMdellequ) * (M_Ar / M ** 2)
            dellmu_H2dellequ = (dellx_H2dellequ * M - x_H2 * dellMdellequ) * (M_H2 / M ** 2)
