...
print(
    f"Number of data points with peak pressure under 300 bar: {np.count_nonzero(mask)}"
)
print(
    f"Number of data points with peak pressure over 300 bar: {np.count_nonzero(mask == 0)}"
)
...
print(
    f"Number of data points with inlet temp under 500 K: {np.count_nonzero(mask == 0)}"
)
y_cleaned = y_cleaned[mask]
X_cleaned = X_cleaned[mask]
...
print(
    f"Number of data points with pressure rise over 1.3: {np.count_nonzero(mask == 0)}"
)
y_cleaned = y_cleaned[mask]
X_cleaned = X_cleaned[mask]
...
print(
    f"Number of data points with bore smaller than 100mm: {np.count_nonzero(mask == 0)}"
)
y_cleaned = y_cleaned[mask]
X_cleaned = X_cleaned[mask]
