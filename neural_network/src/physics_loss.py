...
from thermo import mixture, fuel_props, JETA_L, H2
...
class PhysicsInformedLoss(nn.Module):
    ...
    def energy_conservation_constraint(self, inputs, outputs):
        ...
        with torch.no_grad():  # Don't track gradients through external functions
            inputs_np = inputs_physical.cpu().numpy()
            outputs_np = outputs_physical.cpu().numpy()

            # Extract relevant variables (now in physical units)
            # Inputs: ['p_in', 'T_in', 'PI', 'cr', 'bore', 'v_mean', 'T_fuel', 'far']
            T_in = inputs_np[:, 1]  # Temperature in (K)
            T_fuel = inputs_np[:, 6]  # Fuel temperature (K)
            far = inputs_np[:, 7]  # Fuel-air ratio (dimensionless)
            p_in = inputs_np[:, 0]  # Inlet pressure
            p_ratio = inputs_np[:, 2]
            p_out = p_ratio * p_in  # Outlet pressure

            # Outputs: ['power', 'heat_loss', 'nox', 'p_tdc', 'p_max', 'T_max', 'T_out', 'air_flow']
            power = outputs_np[:, 0]  # Mechanical power (W)
            heat_loss = outputs_np[:, 1]  # Heat loss (W)
            T_out = outputs_np[:, 6]  # Temperature out (K)
            air_flow = outputs_np[:, 7]  # Air mass flow (kg/s)
            ...
            far_stoich, LHV = fuel_props(self.fuel_type)

            equ_in = 0.0

            # Specific energy in, out and fuel enthalpy
            for i in range(h_in.shape[0]):
                h_in_temp, _, _, _, _, _, s, _ = mixture(T_in[i], p_in[i], equ_in, fuel_type=self.fuel_type)
                h_out_temp, _, _, _, _, _, s, _ = mixture(T_out[i], p_out[i], far[i] / far_stoich,
                                                                 fuel_type=self.fuel_type)
                if self.fuel_type == "jetA":
                    _, h_fuel_temp, s, _ = JETA_L(T_fuel[i])
                else:
                    _, h_fuel_temp, s, _ = H2(T_fuel[i])
                h_in[i] = h_in_temp
                h_out[i] = h_out_temp
            ...
            energy_balance_np = H_in + H_fuel - H_out - power - heat_loss

            # normalise energy conservation with LHV
            fuel_energy_in = far * air_flow * LHV
            energy_balance_normalised_np = energy_balance_np / fuel_energy_in

        energy_balance = torch.from_numpy(energy_balance_normalised_np).to(inputs.device)

        # Return squared constraint violation (should be close to zero)
        return torch.mean(energy_balance ** 2)
