...
data = pd.read_csv("./input_data/" + folder + "/data_cleaned.csv", index_col=0)
...
print(f"Number of datapoints with non-zero efficiency: {data.shape[0]}")
...
powers1 = []
for T_in1 in tins:
    temp = model.inference(np.array([p_in, T_in1, PI, cr, bore, v_mean, far]))
    powers1.append(temp[0][0])

fars = np.linspace(0.02923 / 3.0, 0.02923 / 1.5, 1000)

powers2 = []
for far in fars:
    temp = model.inference(np.array([p_in, T_in, PI, cr, bore, v_mean, far]))
