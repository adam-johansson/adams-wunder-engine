...
def match_piston_surrogate(data, meta_model, power_req, core_flow):
    ...
    piston_input_final = np.atleast_2d(
        np.array([pin, Tin, cr, bore, far_final, p_ratio])
    )
    ...
    dummy, dummy, P_circumv_final = components.compressor(
        data[1], data[0] / 0.99, m_circumvent_final, 0.85, data[2] * 0.99 * 0.99
    )
