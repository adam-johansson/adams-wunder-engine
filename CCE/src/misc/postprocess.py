...
def energy_flow_fuel(lhv, m_f_p, m_f_b, t_tank, fuel_type, t_fuel, P_indicated, piston_heatloss,
                     t_p_in, t_p_out, m_p_in, m_p_out, equ_p_out, t_b_in, equ_b_in, m_b_in, t_b_out, equ_b_out,
                     m_b_out):
    """
    ...
    station2 = ('fuel power piston', 'indicated power piston',
                'piston wall heat loss', 'energy through exhaust (diff)', 'gas energy increase piston')

    station3 = ('fuel power burner', 'gas energy increase')

    station4 = ('fuel tank', 'fuel flow into piston', 'air flow into piston', 'mass flow out of piston',
                'mass flow balance', 'mass flow into the burner', 'fuel flow into burner',
                'mass flow out of burner', 'mass balance burner')

    masses = np.array([m_f, m_f_p, m_p_in, m_p_out, m_balance_p, m_b_in, m_f_b, m_b_out, m_balance_b]) * 1e3

    powers_fuel_tank = np.array([P_f_t, P_f_hx, P_f]) * 1e-3
    perc_fuel_tank = np.array([P_f_t / P_f, P_f_hx / P_f, P_f / P_f]) * 100
    power_piston = np.array([P_f_p, P_indicated, piston_heatloss, P_p_gas2, P_p_gas]) * 1e-3
    perc_piston = np.array([P_f_p / P_f_p, fraction_indicated, fraction_wall, fraction_2, fraction_gas]) * 100
