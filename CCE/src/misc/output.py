...
def print_output(airflow_piston, sfc, F, m0, m2, power_tot, power_hpc, power_lpt, p3, T3, p35, T35, T4, p_max,
                 T_max, equ_trapped, far_s, induced_power, fuel_flow_piston, fuel_flow_burner, V_d_tot,
                 friction_loss, aux_loss, heat_loss, bpr_piston):
    ...
    print(f'Induced power from engine: {induced_power * 1e-6} [MW]')
...
def csv_output(p, t, m):
    ...
    stations = ('2', '13', '14', '15', '21', '25', '3', '31', '32', '34', '35', '4', '41', '43', '5', '6')
    stations = np.atleast_2d(stations).T
    data = np.concatenate((stations, np.atleast_2d(m).T, np.atleast_2d(t).T, np.atleast_2d(p).T), axis=1)
...
def plot_stations(p_array, t_array):
    ...
    labels = ['2', '13', '14', '15', '21', '25', '3', '31', '32', '34', '35', '4', '41', '43', '5', '6']
...
def optimisation_csv(sfc, opr, split, pi_pe, cr, bore, fpr, bpr, p_max, T_max, T_in_piston, T_out_piston, TET,
                     far_piston):
    ...
    headers = ('TSFC [mg/Ns]', 'OPR [-]', 'Pressure split (LPC / HPC) [-]', 'Pressure raise piston [-]',
               'CR [-]', 'Cylinder bore [mm]', 'FPR (outer) [-]', 'BPR [-]', 'p_max [bar]', 'T_max [K]', 'T_in_cyl [K]',
               'T_out_cyl [K]', 'TET [K]', 'FAR piston [-]')

    data = np.array([sfc, opr, split, pi_pe, cr, bore, fpr, bpr, p_max, T_max, T_in_piston,
