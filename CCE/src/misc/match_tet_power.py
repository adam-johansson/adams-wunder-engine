...
def match_tet_power(data, meta_model, power_req, core_flow, T35_req):
    ...
    def find_match(x):
        ...
        residual = np.array([(shaft_power - power_req), T35 - T35_req])
    ...
    try:
        start = timer()
        x0 = np.array([0.1, 0.02])  # bore, far
        x_match, infodict, ier, mesg = fsolve(find_match, x0, full_output=True)
        #print(infodict)
        if ier == 1:
            sol = infodict['fvec']
            print(f'Fsolve worked. {sol}, {x_match[0]}, {x_match[1]}')
        end = timer()
    ...
    # input to surrogate
    bore_final = x_match[0]  # far that matched the power
    far34_final = x_match[1]  # far that matched the power

    # get the output of the surrogate
    piston_input_final = np.atleast_2d(np.array([pin, Tin, cr, bore_final, far34_final, p_ratio]))
    ...
    p_circ_final, T_circ_final, P_circumv_final = \
        components.compressor(Tin, pin / 0.99, m_circumvent_final, 0.85, p_ratio * 0.99 * 0.99)
    ...
    # things needed for aux and friction losses
    bsr = data[9]
    stroke = bore_final / bsr
    lv_max = bore_final * 0.1
    cylinders = data[31]
    v_mean = data[10]
    rpm = v_mean / (2 * stroke) * 60
    rps = rpm / 60
    Vd_tot = stroke * bore_final ** 2 / 4 * np.pi * cylinders
    cycle = data[3]
    if cycle == '4T':
        n_r = 2
    else:
    ...
    # auxiliary losses and friction losses. these do not depend on the trapped fuel air ratio
    fmep, fmep_aux, fmep_pe_loss = post_processing.friction_patton(bore_final, rpm, stroke, v_mean, pin, cr, cylinders,
                                                                   lv_max)
    friction_loss = fmep_pe_loss * Vd_tot * rps / n_r  # friction losses for total engine all cylinders
    aux_loss = fmep_aux * Vd_tot * rps / n_r  # auxiliary losses
    ...
    return T34_final, T35_final, p34, p35, m34, m35, far34_final, far35, shaft_power_final,\
        ...
        friction_loss, aux_loss, heat_loss_final, P_fuel_pump_final, bore_final, error
