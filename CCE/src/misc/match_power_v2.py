...
def match_power_v2(data, meta_model, power_req, core_flow):
    ...
    def find_match(x):
        ...
        bore = x[0]  # bore is varied to match target power

        if bore < 0.01 or bore > 0.2:
            residual = np.array([1e6])
            return residual
        ...
        #print(residual, far34)

        return residual

    try:
        start = timer()
        x0 = np.array([0.1])  # bore
    ...
    # input to surrogate
    bore = x_match[0]  # far that matched the power
    p_ratio_final = p_ratio
    far34_final = far34


    # get the output of the surrogate
    piston_input_final = np.atleast_2d(np.array([pin, Tin, cr, bore, far34_final, p_ratio_final]))
    ...
    #print(T35_final)
    #print(p_ratio_final)
    #print(f'Power matching N: {shaft_power_final - power_req}, {bore}, {T35_final}, {T34_final}')
