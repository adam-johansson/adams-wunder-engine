...
def match_piston_engine(data, flags, match, power_req, flow_req):
    ...
    if not match:
        T45, power_piston, eta_th, air_flow, p_max, T_max, far, equ_trapped, induced_power \
            = run_piston_engine(data, flags)

        return T45, power_piston, eta_th, air_flow, p_max, T_max, far, data[8], equ_trapped, data[30], induced_power

    else:  # now the power output from the piston engine has to match the power needed for the hpc
        def find_match(x):
            if x[0] < 0.168 or x[0] > 0.17 or x[1] < 0.028 or x[1] > 0.030:  # first is diameter second far
                print('otu of bounds')
            ...
            T45, power_piston, eta_th, air_flow, p_max, T_max, far, equ_trapped, induced_power, friction_loss, \
                aux_loss, heat_loss = run_piston_engine(data_temp, flags)
            ...
            shaft_power = induced_power - aux_loss - friction_loss  # induced power - friction losses and auxiliaries
        ...
        T45, power_piston, eta_th, air_flow, p_max, T_max, far, equ_trapped, induced_power, friction_loss, \
            aux_loss, heat_loss = run_piston_engine(data, flags)

    return T45, power_piston, eta_th, air_flow, p_max, T_max, far, x_match[0], equ_trapped, x_match[1], induced_power, \
        friction_loss, aux_loss, heat_loss
...
def match_piston_engine_cr(data, flags, match, power_req, flow_req):
    ...
    return T45, power_piston, eta_th, air_flow, p_max, T_max, far, x_match[0], equ_trapped, induced_power
...
def match_piston_new(data, flags, power_req, core_flow):
    ...
    def find_match(x):
        ...
        T45, power_piston, eta_th, air_flow, p_max, T_max, far, equ_trapped, induced_power, friction_loss, \
            aux_loss, heat_loss, p_tdc = run_piston_engine(data_temp, flags)
    ...
    return T45, shaft_power, eta_th, air_flow, p_max, T_max, far, equ_trapped, x_match[0], induced_power, \
        friction_loss, aux_loss, heat_loss, P_fuel_pump
