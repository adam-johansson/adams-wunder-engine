...
def mix(m1, t1, equ1, m2, t2, equ2, fuel_type=False):
    ...
    cp1, h1, s1, M1 = thermo.properties(t1, p, equ1, fuel_type=fuel_type)
    cp2, h2, s2, M2 = thermo.properties(t2, p, equ2, fuel_type=fuel_type)

    h3 = (m1 * h1 + m2 * h2) / m3

    def find_t3(t):
        cp3, h3_guess, s3, M3 = thermo.properties(t[0].astype(float), p, equ3, fuel_type=fuel_type)
        return h3 - h3_guess
