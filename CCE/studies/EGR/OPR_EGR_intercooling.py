...
EGR_massflow = np.zeros((num1,num2))
equ_piston_in = np.zeros((num1,num2))

piston_massflow = np.zeros((num1,num2))
core_massflow = np.zeros((num1,num2))
...
piston_bprs = np.concatenate((params_1.T, piston_bprs), axis=1)
piston_bprs = np.concatenate((params_2, piston_bprs), axis=0)

EGR_massflow = np.concatenate((params_1.T, EGR_massflow), axis=1)
EGR_massflow = np.concatenate((params_2, EGR_massflow), axis=0)

equ_piston_in = np.concatenate((params_1.T, equ_piston_in), axis=1)
equ_piston_in = np.concatenate((params_2, equ_piston_in), axis=0)

piston_massflow = np.concatenate((params_1.T, piston_massflow), axis=1)
piston_massflow = np.concatenate((params_2, piston_massflow), axis=0)

core_massflow = np.concatenate((params_1.T, core_massflow), axis=1)
core_massflow = np.concatenate((params_2, core_massflow), axis=0)
...
np.savetxt(f"./results/{param_name}/BPR_piston.dat", piston_bprs, fmt="%.5f")
np.savetxt(f"./results/{param_name}/EGR_massflow.dat", EGR_massflow, fmt="%.5f")
np.savetxt(f"./results/{param_name}/equ_piston_in.dat", equ_piston_in, fmt="%.5f")
np.savetxt(f"./results/{param_name}/piston_massflow.dat", piston_massflow, fmt="%.5f")
np.savetxt(f"./results/{param_name}/core_massflow.dat", core_massflow, fmt="%.5f")
