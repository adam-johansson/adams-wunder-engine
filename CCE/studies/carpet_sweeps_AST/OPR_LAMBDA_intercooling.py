...
param_name = "OPR_LAMBDA"
params_1 = np.arange(12,26.1,2)
params_2 = np.arange(0.0,0.36,0.05)
...
def evaluate_grid_point(i, j, opr, pr, root_dir):
    lap1 = timer()
    result = _evaluate_grid_point_inner(i, j, opr, pr, root_dir)
    lap2 = timer()
    status = "FAILED" if result["error"] else "ok"
    print(f"[{status}] OPR={opr:.2f}, far={pr:.1f}  (point {i},{j})  --  {lap2 - lap1:.2f} s")
    return result


def _evaluate_grid_point_inner(i, j, opr, pr, root_dir):
    old_cwd = os.getcwd()
    ...
    with tempfile.TemporaryDirectory(dir=root_dir, prefix="worker_") as run_dir:
        try:
            ...
            cce_input_local["PR"] = pr
            cce_input_local["ratio IC"] = 1.0
