...
param_name = "SC_CD"
params_1 = np.arange(340,370.1,10)
params_2 = np.arange(20,80.1,10)
...
def evaluate_grid_point(i, j, phi_sc, phi_cd, root_dir):
    lap1 = timer()
    result = _evaluate_grid_point_inner(i, j, phi_sc, phi_cd, root_dir)
    lap2 = timer()
    status = "FAILED" if result["error"] else "ok"
    print(f"[{status}] phi_sc={phi_sc:.2f}, phi_cd={phi_cd:.1f}  (point {i},{j})  --  {lap2 - lap1:.2f} s")
    return result


def _evaluate_grid_point_inner(i, j, phi_sc, phi_cd, root_dir):
    old_cwd = os.getcwd()
