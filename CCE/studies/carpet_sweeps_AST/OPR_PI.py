...
param_name = "OPR_PI"
params_1 = np.arange(18,25.1,1)
params_2 = np.arange(0.9,1.51,0.15)
...
def evaluate_grid_point(i, j, opr, pi_pe, root_dir):
    lap1 = timer()
    result = _evaluate_grid_point_inner(i, j, opr, pi_pe, root_dir)
    lap2 = timer()
    status = "FAILED" if result["error"] else "ok"
    print(f"[{status}] OPR={opr:.2f}, far={pi_pe:.1f}  (point {i},{j})  --  {lap2 - lap1:.2f} s")
    return result


def _evaluate_grid_point_inner(i, j, opr, pi_pe, root_dir):
    old_cwd = os.getcwd()
