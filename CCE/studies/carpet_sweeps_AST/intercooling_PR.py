...
param_name = "intercooling_PR"
params_1 = np.linspace(0.0,1.0, num)
params_2 = np.linspace(0.0,0.45, num)



SFCs = np.zeros((num,num))

EI_noxs = np.zeros((num,num))
m_noxs_pe = np.zeros((num,num))
m_noxs_burner = np.zeros((num,num))

m_NO_tot = np.zeros((num,num))

pmaxs = np.zeros((num,num))
dT_intercoolers = np.zeros((num,num))
Tmaxs = np.zeros((num,num))
T_max_twozone = np.zeros((num,num))
T4s = np.zeros((num,num))
T34s = np.zeros((num,num))
T35s = np.zeros((num,num))

core_effs = np.zeros((num,num))
thermal_effs = np.zeros((num,num))
transmission_effs = np.zeros((num,num))
propulsive_effs = np.zeros((num,num))
overall_effs = np.zeros((num,num))
gg_effs = np.zeros((num,num))

specific_thrusts = np.zeros((num,num))
specific_powers = np.zeros((num,num))

core_powers = np.zeros((num,num))
gg_powers = np.zeros((num,num))
gg_mass_spec_powers = np.zeros((num,num))
gg_disp_spec_powers = np.zeros((num,num))
cooling_ratios = np.zeros((num,num))

hot_bypass_thrusts = np.zeros((num,num))
cold_bypass_thrusts = np.zeros((num,num))
core_thrusts = np.zeros((num,num))

piston_fuelflow = np.zeros((num,num))
burner_fuelflow = np.zeros((num,num))

cool_ngv = np.zeros((num,num))
cool_rotor = np.zeros((num,num))
m_core = np.zeros((num,num))

bprs = np.zeros((num,num))
bores = np.zeros((num,num))

piston_bprs = np.zeros((num,num))
piston_power_spec = np.zeros((num,num))


piston_powers = np.zeros((num,num))
piston_heatloss = np.zeros((num,num))
piston_powers_indicated = np.zeros((num,num))
heatloss_percentage = np.zeros((num,num))
friction_percentage = np.zeros((num,num))
...
for IC_ratio in params_1:
    ...
    for pr in params_2:
        ...
        if output_dict["error"]:
            # probably no bore is found to match power
            print("hej")
            error = True
        else:
