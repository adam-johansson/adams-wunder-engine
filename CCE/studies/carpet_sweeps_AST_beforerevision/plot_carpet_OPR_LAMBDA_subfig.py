...
data = np.loadtxt("./results/OPR_LAMBDA/thermal_eff.dat")

OPR = data[1:,0]
T4 = data[0,1:]

eff = data[1:,1:]
nox = np.loadtxt("./results/OPR_LAMBDA/specific_nox.dat")[1:,1:]
power = np.loadtxt("./results/OPR_LAMBDA/core_spec_power.dat")[1:,1:]
pmax = np.loadtxt("./results/OPR_LAMBDA/peak_pressure.dat")[1:,1:]
bore = np.loadtxt("./results/OPR_LAMBDA/bore.dat")[1:,1:]
Tout = np.loadtxt("./results/OPR_LAMBDA/Tout_piston.dat")[1:,1:]
...
fig.add_annotation(
    ...
    ay=1.3,    # text box y (in data coordinates)
    text="d > 200 mm",
    showarrow=True,
...
fig.add_annotation(
    ...
    y=0.83,     # arrowhead y
    ax=120,      # text box x (in data coordinates)
...
fig.add_annotation(
    ...
    textangle=53,  # rotate to follow axis
)

fig.add_annotation(
    x=38,        # negative x puts it outside the plot to the left
    y=1.22,
    #xref="paper",
    #yref="paper",
    text="OPR [-]",
...
fig.update_layout(
    margin=dict(l=0, r=0, t=0, b=120, pad=10),
)
...
fig.write_image("OPR_LAMBDA_plot_subfig.pdf", width=550, height=800, scale=2)
fig.show()