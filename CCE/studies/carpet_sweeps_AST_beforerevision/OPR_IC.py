...
params_2 = np.arange(0.0,1.1,0.25)
...
def evaluate_grid_point(i, j, opr, ratio_IC, root_dir):
    lap1 = timer()
    result = _evaluate_grid_point_inner(i, j, opr, ratio_IC, root_dir)
    lap2 = timer()
    status = "FAILED" if result["error"] else "ok"
    print(f"[{status}] OPR={opr:.2f}, far={ratio_IC:.1f}  (point {i},{j})  --  {lap2 - lap1:.2f} s")
    return result


def _evaluate_grid_point_inner(i, j, opr, ratio_IC, root_dir):
    old_cwd = os.getcwd()
    ...
    with tempfile.TemporaryDirectory(dir=root_dir, prefix="worker_") as run_dir:
        try:
            ...
            cce_input_local["ratio IC"] = ratio_IC
