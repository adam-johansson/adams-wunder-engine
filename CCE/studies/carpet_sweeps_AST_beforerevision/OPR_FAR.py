...
def evaluate_grid_point(i, j, opr, far, root_dir):
    lap1 = timer()
    result = _evaluate_grid_point_inner(i, j, opr, far, root_dir)
    lap2 = timer()
    status = "FAILED" if result["error"] else "ok"
    print(f"[{status}] OPR={opr:.2f}, far={far:.1f}  (point {i},{j})  --  {lap2 - lap1:.2f} s")
    return result


def _evaluate_grid_point_inner(i, j, opr, far, root_dir):
    old_cwd = os.getcwd()
