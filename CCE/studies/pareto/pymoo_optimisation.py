...
class MyEngineProblem(Problem):
    def __init__(self):
        super().__init__(n_var=7,  #OPR, T4, split, cr, far, pi_pe, ic ratio
                            ...
                            xu=np.array([30, 1600, 0.5, 15, 5, 1.5, 1.0])) #upp limit on the variables
...
class OptimisationCallback(Callback):
    def __init__(self, ref_point, gen_history=None, hv_history=None):
        super().__init__()
    ...
    def notify(self, algorithm):
        ...
        hv_df.to_csv("optimisation_data/hypervolume.csv", index=False)
...
if resume_optimisation and os.path.exists("optimisation_data/all_evaluations.csv"):
    existing_df = pd.read_csv("optimisation_data/all_evaluations.csv")
    all_evaluations.extend(existing_df.to_dict('records'))


# --- Load existing hypervolume history if it exists ---
hv_csv_path = "optimisation_data/hypervolume.csv"
...
ref_point = np.array([0.0, 2.0])  # adjust nox ref based on your data range
...
# continue opitimisation
    ...
    pop_X = np.loadtxt("optimisation_data/last_population_X.csv", delimiter=",")
    pop_F = np.loadtxt("optimisation_data/last_population_F.csv", delimiter=",")
    gen_done = int(np.loadtxt("optimisation_data/last_generation.csv", delimiter=","))
...
pop_X = res.pop.get("X")
pop_F = res.pop.get("F")
np.savetxt("optimisation_data/last_population_X.csv", pop_X, delimiter=",")
np.savetxt("optimisation_data/last_population_F.csv", pop_F, delimiter=",")

# Save total generations done (not just this run's count)
gen_offset = gen_done if resume_optimisation else 0
np.savetxt("optimisation_data/last_generation.csv", [res.algorithm.n_gen + gen_offset], delimiter=",")
