...
param_name = "wiebe"  # start of combustion only
params_1 = np.arange(0.1, 5.1, 0.1)  # phi_sc sweep -- everything else fixed
...
def evaluate_grid_point(i, m_wiebe, root_dir):
    lap1 = timer()
    result = _evaluate_grid_point_inner(i, m_wiebe, root_dir)
    lap2 = timer()
    status = "FAILED" if result["error"] else "ok"
    print(f"[{status}] m_wiebe={m_wiebe:.2f}  (point {i})  --  {lap2 - lap1:.2f} s")
    return result


def _evaluate_grid_point_inner(i, m_wiebe, root_dir):
    old_cwd = os.getcwd()
